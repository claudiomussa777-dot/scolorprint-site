...
def slide_one() -> Image.Image:
    ...
    wrapped(
        draw,
        "Entrada, fundo, mensagem e brinde podem ser decididos hoje, antes da correria da semana.",
        68,
        706,
        24,
        28,
    ...
    draw.rounded_rectangle((68, 1012, 470, 1090), radius=24, fill=LIME)
    draw.text((98, 1038), "GUARDE PARA PLANEAR HOJE", font=font(BOLD, 24), fill=NAVY)
    footer(draw, "1/5")
    return canvas
...
def slide_two() -> Image.Image:
    ...
    bullet_list(
        draw,
        [
            "Backdrop ajuda quando vai existir zona de foto, palco ou recepcao.",
            "Tambem funciona bem se a marca precisa de aparecer com mais consistencia nas imagens.",
            "Planeie primeiro o local onde ele vai ficar para evitar improviso no dia.",
        ],
        68,
        640,
        22,
    ...
    info_card(draw, (64, 1014, 548, 1178), "Pergunta util", "Vai precisar de zona de foto, palco ou recepcao?")
    footer(draw, "2/5")
    return canvas
...
def slide_three() -> Image.Image:
    ...
    bullet_list(
        draw,
        [
            "Roll-up ajuda a orientar, acolher ou reforcar o tema principal do encontro.",
            "Vale a pena definir a frase principal antes de enviar o ficheiro.",
            "Se a recepcao ou corredor precisa de leitura rapida, costuma resolver bem.",
        ],
        68,
        642,
        22,
    ...
    info_card(draw, (64, 1016, 548, 1204), "Planeie antes", "Uma mensagem principal costuma bastar para a leitura imediata.")
    footer(draw, "3/5")
    return canvas
...
def slide_four() -> Image.Image:
    ...
    bullet_list(
        draw,
        [
            "Brindes uteis, como canetas, ajudam a prolongar o contacto depois do evento.",
            "Vinil na entrada ou montra reforca a chegada e prepara melhor a recepcao.",
            "Quando fecha estes detalhes no domingo, segunda-feira comeca com mais clareza.",
        ],
        68,
        614,
        21,
...
def slide_five() -> Image.Image:
    ...
    info_card(draw, (64, 612, 476, 786), "Entrada", "Vinil ou peca de acolhimento para a chegada.")
    info_card(draw, (64, 804, 476, 978), "No espaco", "Backdrop ou roll-up ja alinhados com a mensagem.")
    info_card(draw, (64, 996, 476, 1170), "No fecho", "Brinde util ou pedido de orcamento preparado.")
    draw.rounded_rectangle((612, 1112, 970, 1210), radius=26, fill=(255, 253, 247, 236), outline=(17, 23, 44, 35), width=2)
    wrapped(draw, "Comente 'evento' e planeamos consigo.", 638, 1138, 24, 20, NAVY, bold=True, spacing=6)
    footer(draw, "5/5")
    return canvas
